...
//        filter.addAction(Intent.ACTION_BATTERY_CHANGED)
    ...
    private fun createLockScreen() {
        ...
        view = View.inflate(applicationContext, com.vunhiem.lockscreenios.R.layout.lock_layout, mView)
...
//        Log.i("Title", title)
        ...
        if (pack != "android" && pack != "com.android.systemui" && pack != "android.providers.downloads" && title != null && title!="iOS Lock active") {
            ...
            if (intent.action == Intent.ACTION_SCREEN_OFF) {
                ...
                if (switchMainStatus == true) {
                    ...
                    if (idWallpaper != null) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
                            imgBackgroundLock?.let {
                                Glide.with(applicationContext).load(AppConfig.getIdWallPaper(applicationContext)!!.toInt()).into(
                                    it
                                )
                            }
